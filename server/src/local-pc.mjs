...
const manager = new SessionManager({
  ...
  runner: new WindowsEmulatorRunner({ biosDirectory: process.env.BIOS_DIRECTORY ?? path.join(root, "bios-private") })
});
...
http.createServer(async (request, response) => {
  try {
    ...
    if (request.method === "GET" && url.pathname === "/health") return reply(response, 200, { ok: true, mode: "local-pc", webrtc: "pending" });
...
}).listen(port, bindAddress, () => console.log(`Gran Z Retro local-pc API listening on http://${bindAddress}:${port}`));
